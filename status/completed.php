...
<head>
    <title>Completed Anime on AnimeCode</title>
    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    <meta name="title" content="Completed Anime on AnimeCode">
    <meta name="description" content="Completed Anime in HD with No Ads. Watch anime online">
    ...
    <meta property="og:title" content="Completed Anime on AnimeCode">
    <meta property="og:description" content="Completed Anime on AnimeCode in HD with No Ads. Watch anime online">
                    ...
                    <section class="block_area block_area_category">
                        <div class="block_area-header">
                            <div class="float-left bah-heading mr-4">
                                <h2 class="cat-heading">Completed Anime</h2>
                            </div>
                            <div class="clearfix"></div>
                        </div>
                        <div class="tab-content">
                            <div class="block_area-content block_area-list film_list film_list-grid film_list-wfeature">
                                ...
                                <?php 
								 $ch = curl_init();
								  curl_setopt($ch, CURLOPT_URL, "$api/completed-anime?page=$page");
								  curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
								  curl_setopt($ch, CURLOPT_HEADER, FALSE);
								  curl_setopt($ch, CURLOPT_HTTPHEADER, array("Content-Type: application/json"));
								  $response = curl_exec($ch);
								  $json = json_decode($response, true);
                               // $json = file_get_contents("$api/completed-anime?page=$page");
                               // $json = json_decode($json, true);
                                foreach((array) $json as $key => $completed) { ?>
                                    <div class="flw-item">
                                        <div class="film-poster">
                                            <div class="tick ltr">
                                                <div class="tick-item-<?php $str = $completed['animeTitle'];
                                                  $last_word_start = strrpos ( $str , " ") + 1;
                                                  $last_word_end = strlen($str) - 1;
                                                  $last_word = substr($str, $last_word_start, $last_word_end);
                                                  if ($last_word == "(Dub)"){echo "dub";} else {echo "sub";}
                                                ?>  tick-eps amp-algn"><?php $str = $completed['animeTitle'];
                                                $last_word_start = strrpos ( $str , " ") + 1;
                                                $last_word_end = strlen($str) - 1;
                                                $last_word = substr($str, $last_word_start, $last_word_end);
                                                if ($last_word == "(Dub)"){echo "Dub";} else {echo "Sub";}
                                            ...
                                            <img class="film-poster-img lazyload"
                                                data-src="https://ik.imagekit.io/<?=$imgk?>/tr:f-webp/<?=$completed['imgUrl']?>"
                                                src="<?=$cdn?>/images/no_poster.jpg"
                                                alt="<?=$completed['animeTitle']?>">
                                            <a class="film-poster-ahref"
                                                href="/anime/<?=$completed['animeId']?>"
                                                title="<?=$completed['animeTitle']?>"
                                                data-jname="<?=$completed['animeTitle']?>"><i class="fas fa-play"></i></a>
                                        </div>
                                        <div class="film-detail">
                                            <h3 class="film-name">
                                                <a
                                                    href="/anime/<?=$completed['animeId']?>"
                                                    title="<?=$completed['animeTitle']?>"
                                                    data-jname="<?=$completed['animeTitle']?>"><?=$completed['animeTitle']?></a>
                                            </h3>
                                            <div class="description"></div>
                                            <div class="fd-infor">
                                                <span class="fdi-item"><?=$completed['status']?></span>
                                                <span class="dot"></span>
                                                <span class="fdi-item">Completed</span>
                                            </div>
                                        </div>
                                        <div class="clearfix"></div>
                                    </div>
                                ...
                                <div class="pagination">
                                    <nav>
                                        ...
                                        <?php 
										 $ch = curl_init();
								  curl_setopt($ch, CURLOPT_URL, "$api/completedPage?page=$page");
								  curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
								  curl_setopt($ch, CURLOPT_HEADER, FALSE);
								  curl_setopt($ch, CURLOPT_HTTPHEADER, array("Content-Type: application/json"));
								  $completedPage = curl_exec($ch);
								  $completedPage = json_decode($completedPage, true);
                                         //  $completedPage = file_get_contents("$api/completedPage?page=$page");
                                          // $completedPage = json_decode($completedPage, true); 
										   { ?>
                                             <?=$completedPage['pagination']; ?>
                                           <?php } curl_close($ch); ?>
                                        </ul>
                                    </nav>
                                </div>
